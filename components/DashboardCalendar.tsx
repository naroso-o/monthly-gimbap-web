...
export function DashboardCalendar({ periodId }: { periodId: string }) {
  ...
  return (
    <div className="bg-diary-card border border-diary-border rounded-diary p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 text-xs text-diary-muted">
          ...
          <div className="flex items-center gap-1">
            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: "#E6B887" }}></div>
            <span>30분미만</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: "#8B7355" }}></div>
            <span>2시간이상</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {["일", "월", "화", "수", "목", "금", "토"].map((day) => (
          <div key={day} className="text-center text-xs text-diary-muted py-1">
            {day}
          </div>
        ))}
      </div>
        ...
        {days.map((day, index) => {
          ...
          return (
            <div key={index} className="aspect-square flex items-center justify-center relative">
              ...
              {/* 오늘 표시 */}
              {isToday && <div className="absolute inset-0 border border-diary-accent rounded-full opacity-50" />}
            </div>
          )
        })}
      </div>
          ...
          <span className="text-diary-text font-medium">
            {Math.round(
              Object.values(mockAttendanceData)
                ...
                Object.values(mockAttendanceData).filter((data) => data.attended).length,
            )}
            분
          </span>
        </div>
