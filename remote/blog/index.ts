...
export const useCreatePostMutation = () => {
  ...
  return useMutation({
    mutationFn: async (post: CreatePost) => {
      if (!user || !period) throw new Error("User or period not found");
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["blog_post_check", period?.id, user?.id],
      });
      queryClient.invalidateQueries({
        queryKey: ["blog_posts", period?.id, user?.id],
      });
      queryClient.invalidateQueries({
        queryKey: ["total_blog_posts", period?.id, user?.id],
      });
    },
  });
};
